...
RUN chmod +x ${BUILD_TOOLS_DIR}/preprocess.sh
...
RUN sed -i 's/\r$//' ${BUILD_TOOLS_DIR}/preprocess.sh ${BUILD_TOOLS_DIR}/build.sh ${BUILD_TOOLS_DIR}/run.sh
ENTRYPOINT ["/bin/bash", "-c", "${BUILD_TOOLS_DIR}/preprocess.sh && ${BUILD_TOOLS_DIR}/build.sh && ${BUILD_TOOLS_DIR}/run.sh"]